...
message_sizes=(50 1024 10240) #change the performance-report.py  
garbage_collectors=(UseSerialGC UseParallelGC UseG1GC UseConcMarkSweepGC)    

jtl_location=/home/ubuntu/pasindu/jtls

ballerina_host_user=ubuntu@172.31.9.39 #to be changed
ballerina_host=172.31.9.39 #to be changed
...
jmx_file=/home/ubuntu/pasindu/jmx/ballerina_merge.jmx
...
performance_report_output_file=/home/ubuntu/pasindu/BallerinaMergePerformance.csv
        ...
        do
		    ...
		    do
			    echo "Checking service"
    			    response_code=$(curl -s -o /dev/null -w "%{http_code}" http://${ballerina_host}:9090/merge/params?number=10)
