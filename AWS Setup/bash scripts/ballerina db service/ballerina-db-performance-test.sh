...
heap_sizes=(800m 1g 4g 8g) #change python script accordingly
...
ballerina_host_user=ubuntu@172.31.10.195 # to be changed
ballerina_host=172.31.10.195 #to be changed
...
start_up_jmx_file=/home/ubuntu/pasindu/jmx/ballerina_db_fill.jmx

jmx_file=/home/ubuntu/pasindu/jmx/ballerina_db.jmx
...
performance_report_output_file=/home/ubuntu/pasindu/BallerinaDBPerformance.csv
...
do
    ...
    do
        ...
        do
		    ...
		    nohup ssh -i "pasindut.pem" -n -f ${ballerina_host_user} "/bin/bash $target_script ${heap} ${total_users} ${target_gc_logs_path} ${gc} ${size}" &

		    echo "Populating DB"
		    while true 
		    do
			    echo "Checking service"
    			    response_code=$(curl -s -o /dev/null -w "%{http_code}" http://${ballerina_host}:9090/db/add)
		    ...
		    ${jmeter_path}/jmeter  -Jgroup1.host=${ballerina_host}  -Jgroup1.port=9090 -n -t ${start_up_jmx_file}
		    ...
		    echo "Finished populating DB"
        	    ...
        	    # Start JMeter server
        	    ${jmeter_path}/jmeter  -Jgroup1.host=${ballerina_host}  -Jgroup1.port=9090 -Jgroup1.threads=$u -Jgroup1.seconds=${test_duration}  -n -t ${jmx_file} -l ${report_location}/results.jtl
                    
                    echo "Running Uptime command"	

                    nohup ssh -i "pasindut.pem" -n -f ${ballerina_host_user} "/bin/bash $target_uptime_script ${heap} ${total_users} ${target_uptime_path} ${gc} ${size}" &
