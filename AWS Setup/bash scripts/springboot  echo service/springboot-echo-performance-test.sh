...
springboot_host_user=ubuntu@172.31.10.206 
springboot_host=172.31.10.206
...
jmx_file=/home/ubuntu/pasindu/jmx/springboot_echo.jmx
...
performance_report_output_file=/home/ubuntu/pasindu/SpringBootEchoPerformance.csv
...
do
    ...
    do
        ...
        do
		    ...
		    nohup ssh  -n -f ${springboot_host_user} -i pasindut.pem "/bin/bash $target_script ${heap} ${total_users} ${target_gc_logs_path} ${gc} ${size}" &
		    ...
		    message=$(<${payloads_output_file_root}/${payload_files_prefix}${size})
        	    ...
        	    ${jmeter_path}/jmeter  -Jgroup1.host=${springboot_host}  -Jgroup1.port=9000 -Jgroup1.threads=$u -Jgroup1.seconds=${test_duration} -Jgroup1.data=${message} -n -t ${jmx_file} -l ${report_location}/results.jtl
                    ...
                    nohup ssh  -n -f ${springboot_host_user} -i pasindut.pem "/bin/bash $target_uptime_script ${heap} ${total_users} ${target_uptime_path} ${gc} ${size}" &
